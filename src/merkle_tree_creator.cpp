...
MerkleTreeCreator::MerkleTreeCreator(const std::string& _contractAddress, std::shared_ptr<Provider> _provider)
...
Transaction MerkleTreeCreator::updateRewardsMerkleRoot() {
    ...
    tx.data = functionSelector + getRoot();

    return tx;
}

Transaction MerkleTreeCreator::validateProof(size_t index, int64_t amount) {
